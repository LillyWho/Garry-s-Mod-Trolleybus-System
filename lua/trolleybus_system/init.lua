...
Trolleybus_System.NoclippingPlayers = Trolleybus_System.NoclippingPlayers or {}
...
if not file.Exists( "trolleybussystem", "DATA" ) then file.CreateDir( "trolleybussystem" ) end
--[[------------------------------------
...
function Trolleybus_System.WriteDataFile( type, data )
	if Trolleybus_System.DefaultMaps[ game.GetMap() ] and not Trolleybus_System.IsEditingDefaultMapsAllowed() then return end
	if not file.Exists( "trolleybussystem/" .. game.GetMap(), "DATA" ) then file.CreateDir( "trolleybussystem/" .. game.GetMap() ) end
...
function Trolleybus_System.ReadDataFile( type )
	if Trolleybus_System.DefaultMaps[ game.GetMap() ] and not Trolleybus_System.IsEditingDefaultMapsAllowed() then return file.Read( "trolleybus_system/data/" .. game.GetMap() .. "/" .. type .. ".lua", "LUA" ) end
	return file.Read( "trolleybussystem/" .. game.GetMap() .. "/" .. type .. ".txt" ) or file.Read( "trolleybus_system/data/" .. game.GetMap() .. "/" .. type .. ".lua", "LUA" )
end
...
function Trolleybus_System.UpdateTransmit( self, setting )
	if not self.UpdateTransmit or CurTime() - self.UpdateTransmit > 1 then
		self.UpdateTransmit = CurTime()
		for k, v in ipairs( player.GetHumans() ) do
			local dist = Trolleybus_System.GetPlayerSetting( v, setting )
			Trolleybus_System.SetPreventTransmit( self, v, Trolleybus_System.EyePos( v ):DistToSqr( self:GetPos() ) > dist * dist or not v:TestPVS( self ) )
		end
	end
end

function Trolleybus_System.AddEntityToAvoid( entClass )
	if not Trolleybus_System.AvoidSpawnInEntClass then Trolleybus_System.AvoidSpawnInEntClass = {} end
	Trolleybus_System.AvoidSpawnInEntClass[ entClass ] = true
end
...
function Trolleybus_System.SaveTrafficTracks( id )
	...
	if id then
		Trolleybus_System.SendMassiveData( nil, {
			id = id,
			track = Trolleybus_System.TrafficTracks[ id ]
		}, "Trolleybus_System.TrafficTrackUpdate" )
	else
...
function Trolleybus_System.LoadTrafficTracks( ply, force )
	...
	if not str and not force then return end
	if not ply then
...
function Trolleybus_System.ForceFlyOffPole( self, right )
	...
	if object then object:OnWheelLeave( self, wire ) end
end
...
hook.Add( "KeyPress", "Trolleybus_System", function( ply, key )
	if key == IN_ATTACK then
		...
		if button and IsValid( bus ) and bus.IsTrolleybus then bus:TryPressButtonBy( ply, button, ply:InVehicle() and 2 or 1 ) end
	end
end )
		...
		for k, v in pairs( bus.m_ButtonsData ) do
			if v.hotkey and bus:CanButtonBePressedBy( ply, k ) and bus:CanButtonBePressedByType( ply, k, 4 ) then
				...
				if istable( keys ) then
					if not table.HasValue( keys, key ) then continue end
					c = #keys
				else
					if keys ~= key then continue end
				end

				if not count or c > count then btn, count = k, c end
			end
		end

		if btn then bus:TryPressButtonBy( ply, btn, 4 ) end
	end
end )
...
hook.Add( "PlayerLeaveVehicle", "Trolleybus_System", function( ply, veh )
	if IsValid( veh ) then
		...
		if IsValid( bus ) then
			local pos = veh.ExitPos and bus:LocalToWorld( veh.ExitPos ) or util.TraceLine( {
				start = veh:GetPos(),
				endpos = veh:GetPos() - Vector( 0, 0, 100 ),
				filter = { ply, veh }
			} ).HitPos
...
net.Receive( "TrolleybusSystem_CreateBus", function( len, ply )
	...
	if not ENT or not Trolleybus_System.IsTrolleybusMetatable( ENT ) then return end
	if updating then
		if ply.Trolleybus_System_UpdateBusTime and CurTime() - ply.Trolleybus_System_UpdateBusTime < 1 then return end
		if not ply:InVehicle() then return end
		local bus = Trolleybus_System.GetSeatTrolleybus( ply:GetVehicle() )
		if not IsValid( bus ) or bus:GetClass() ~= class or bus.Owner ~= ply or not Trolleybus_System.PlayerInDriverPlace( bus, ply ) then return end
	...
	if not hook.Run( "PlayerSpawnSENT", ply, class ) then return end
	local count = 0
	...
	undo.SetPlayer( ply )
	undo.AddEntity( ent )
	if ENT.PrintName then undo.SetCustomUndoText( "Undone " .. ENT.PrintName ) end
...
concommand.Add( "trolleybus_reload_data", function( ply, cmd, args, str )
	if IsValid( ply ) and not Trolleybus_System.RunEvent( "HasAccessToAdminSettings", ply ) then return end
	Trolleybus_System.ContactNetwork.Load()
