...
net.Receive( "Trolleybus_System.ContactNetworkEditor", function( len, ply )
	...
	if not tool or not tool:Allowed() or not IsValid( ply:GetActiveWeapon() ) or ply:GetActiveWeapon():GetClass() ~= "gmod_tool" or ply:GetActiveWeapon().Mode ~= "trolleybus_cn_editor" then return end
	...
	elseif cmd == 2 then
		...
		if object then upd[ name ] = Trolleybus_System.ContactNetwork.GetObjectData( name ) end
	elseif cmd == 3 then
		local name = net.ReadString()
		local object = Trolleybus_System.ContactNetwork.GetObject( name )
		if object then
	...
	elseif cmd == 7 then
		...
		for k, v in pairs( data ) do
			for k2, v2 in pairs( v.Connectables ) do
				if v2[ 3 ] then v.Object:ConnectConnectableTo( k2, data[ v2[ 3 ] ].Object, v2[ 4 ] ) end
			end
		end
