...
local function constructEntitiesToAvoid()
	...
	for _, ent in ents.Iterator() do
		if dontSpawnInSpecifiedEntitiesList( ent.BaseClass ) or dontSpawnInSpecifiedEntitiesList( ent ) then table.insert( entities, ent ) end
	end
	return entities
end
...
local function getVehiclePositionsAndRadii()
	...
	for j = 1, #vehicles do -- get trolleybus AI card
		vehPos[ j ] = vehicles[ j ]:WorldSpaceCenter()
		vehRad[ j ] = vehicles[ j ]:BoundingRadius()
	end

	local tbuses = ents.FindByClass( "trolleybus_ent_*" ) -- get trolleybus entities
	for j = 1, #tbuses do
		busPos[ busPos ] = tbuses[ j ]:WorldSpaceCenter()
		busRad[ busRad ] = tbuses[ j ]:BoundingRadius()
	end

	local otherVehicles = findAllVehicleEntities()
	for j = 1, #otherVehicles do -- get regular vehicle entities
		oVehPos[ j ] = otherVehicles[ j ]:WorldSpaceCenter()
		oVehRad[ j ] = otherVehicles[ j ]:BoundingRadius()
	end

	local avoidEntities = constructEntitiesToAvoid()
	for j = 1, #avoidEntities do -- get any manually registered entities
		entPos[ j ] = otherVehicles[ j ]:WorldSpaceCenter()
		entRad[ j ] = otherVehicles[ j ]:BoundingRadius()
	end

	table.Add( finalPos, vehPos )
	table.Add( finalPos, busPos )
	table.Add( finalPos, oVehPos )
	table.Add( finalPos, entPos )
	table.Add( finalRad, vehRad )
	table.Add( finalRad, busRad )
	table.Add( finalRad, oVehRad )
	table.Add( finalRad, entRad )
	return finalPos, finalRad
end
...
hook.Add( "Think", "Trolleybus_System_Traffic", function()
	...
	if Trolleybus_System.GetAdminSetting( "trolleybus_traffic_enable" ) and #vehicles < VEHICLE_LIMIT and #vehicles < VEHICLE_LIMIT_PER_PLAYER * #plys then
		...
		for i = 1, math.min( SPAWNS_PER_UPDATE, VEHICLE_LIMIT - #vehicles ) do
			allveh_pos, allveh_radius = getVehiclePositionsAndRadii() -- patch in new function to construct a list of all trolleybusses, trolleybus AI, and custom entities
			...
			for trackid, spawnsegments in pairs( add ) do
				...
				PrepareSpawnSegments( spawnsegments, nospawnsegments, avoidEntities )
			...
			for j = 1, #selected do
				...
				if rand < 0 then
					...
					allveh_pos, allveh_radius = getVehiclePositionsAndRadii()
					break
				end
			end
		end
