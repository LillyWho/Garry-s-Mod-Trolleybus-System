...
function ENT:Initialize()
	...
	if not self.CurTrack then self:FindTrack() end
	self.LastData = {
...
function ENT:SetupVehicleClass()
	if self:GetVehicleClass() == "" then
		...
		for k, v in pairs( Trolleybus_System.TrafficVehiclesTypes ) do
			...
			if not v.IsPublicTransport and self:GetTrack() and self:GetTrack().PublicTransportOnly then -- Only spawn on this track if this is a regular vehicle or a public transport vehicle on public transport lane
				continue
			end
...
function ENT:FindTrack()
	...
	for k, v in pairs( Trolleybus_System.GetTrafficTracks() ) do
		...
		if not dist or d < dist then cur, dist = k, d end
	end

	local prevcur = self.CurTrack
	self.CurTrack = cur
	self.UpdateTrackTime = CurTime()
	if cur and prevcur ~= cur then self:SelectNextTrack() end
end
...
function ENT:CalculateCollisionTimePos( ent )
	...
	if not collisionpos then return collisionpos, math.huge end
...
function ENT:SetupJamTrace( ontrack )
	...
	ProtectedCall( function()
		...
		if ontrack and not self.ReverseActive then
			...
			while curtrack and curdist < tracedist do
				...
				traceparts[ #traceparts + 1 ] = {
					Start = self.JamTrace.StartPos,
					End = self.JamTrace.HitPos
				}
...
function ENT:CheckLoopStuck()
	local ent = self
	local loop = {}
	while not loop[ ent ] and ent.JamTrace and ent.JamTrace.Hit and IsValid( ent.JamTrace.Entity ) do
		loop[ ent ] = true
...
function ENT:GetSqrDistToStop()
	...
	if self.JamTrace and self.JamTrace.Hit then dists[ #dists + 1 ] = self.JamTrace.DistToHit ^ 2 end
	if not self.ReverseActive then
		if not Trolleybus_System.TrafficCarLookAroundTime or CurTime() - Trolleybus_System.TrafficCarLookAroundTime > 0.25 then
			...
			for k, v in ipairs( ents.GetAll() ) do
				if not self:IsLookAroundEnt( v ) or v:GetVelocity():Length2DSqr() < 50 * 50 then continue end
				if v:GetClass() == "trolleybus_traffic_car" then
					...
					for k2, v2 in pairs( Trolleybus_System.GetTrafficTracks() ) do
						...
						if math.abs( lang.y ) <= 40 and lpos.y ^ 2 + lpos.z ^ 2 < radius ^ 2 and lpos.x > -radius and lpos.x < v2.Start:Distance( v2.End ) + radius then t[ k2 ] = true end
					end
				end
			end
		end
		...
		if ntrack then
			...
			if IsValid( light ) and light:IsStopSignal() then
				...
				if dist <= tracedist * tracedist then dists[ #dists + 1 ] = dist end
			end
...
function ENT:GetAllowedSpeed( pos )
	...
	if not self.ReverseActive then
		...
		if dist < tracedist and track then
			...
			repeat
				...
				track = #track.Next == 1 and Trolleybus_System.GetTrafficTracks()[ track.Next[ 1 ] ] or nil
			until not track or curdist >= tracedist
	...
	if lang.p > 0 and lang.p < 90 then spd = spd / math.atan( math.rad( 90 - math.min( lang.p, 60 ) ) ) end
...
function ENT:GetSurfaceFriction()
			...
			for k, v in ipairs( v.Wheels ) do
				if v.LastFrictionSnapshot then
					for k, v in ipairs( v.LastFrictionSnapshot ) do
						...
						if sdata and ( not friction or friction > sdata.friction ) then friction = sdata.friction end
					end
				end
			end
		end
	...
	local tr = util.TraceLine( {
		start = ldata.Pos,
		endpos = ldata.Pos - ldata.Ang:Up() * 300,
		filter = self
	} )
...
function ENT:SetupWheels()
	...
	for k, v in ipairs( data.Wheels ) do
		...
		for k2, v2 in ipairs( v.Wheels ) do
			...
			if v.Drive then data.RotationData.Diff[ k2 ] = 0 end
			if v.Turn then
				turnpsum:Add( v2.Pos )
...
function ENT:CalcWheelFinalRotationSpeed( wheel, rspeed, dt )
	if not wheel.WheelsGroup then return end
...
function ENT:TraceHull( startpos, endpos, filter )
	...
	min:Div( div )
	max:Div( div )
	local bounds = { min, Vector( min.x, min.y, max.z ), Vector( min.x, max.y, min.z ), Vector( max.x, min.y, min.z ), max, Vector( max.x, min.y, max.z ), Vector( max.x, max.y, min.z ), Vector( min.x, max.y, max.z ), }
	...
	local tr = util.TraceHull( {
		...
		ignoreworld = true,
		filter = function( ent )
			if ent == self or not self:IsObstacleEnt( ent ) or filter and not filter( ent ) then return false end
			if customcoll and not gamemode.Call( "ShouldCollide", self, ent ) then return false end
			return true
		end
	} )
...
function ENT:ControlAI( task )
	...
	if math.abs( lang.r ) < 90 then
		...
		if not ontrack and not self.ReverseActive then
		...
		if self:CheckLoopStuck() then
			if jamtrace.Entity.JamTrace.Entity == self then
				...
				self:MoveTo( pos )
				if self:GetSpeed() > 1 then self.StuckTime = CurTime() end
			end
		else
		...
		task:Yield()
		if self.ReverseActive then
			if math.abs( self:GetTurn() ) < 10 then self.ReverseActive = false end
	...
	if self.phys:IsMotionEnabled() then
		...
		if self.ReverseTime and CT - self.ReverseTime > 2 then
			self.ReverseActive = not self.ReverseActive
			self.ReverseTime = CT
		end
	else
		self.FlippedTime = CT
