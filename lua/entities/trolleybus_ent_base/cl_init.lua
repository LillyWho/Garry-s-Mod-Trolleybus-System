...
local function CreatePassenger(self)
	for k, v in RandomPairs(self.PassPositionSeats) do
		if not IsValid(self.PassSeats[k]) then
...
function ENT:Initialize()
	...
	self.m_PanelsData = {}
	for k, v in pairs(self.PanelsData or {}) do
		self.m_PanelsData[k] = v
	end

	self.m_ButtonsData = {}
	for k, v in pairs(self.ButtonsData or {}) do
		self.m_ButtonsData[k] = v
	end

	self.m_OtherPanelEntsData = {}
	for k, v in pairs(self.OtherPanelEntsData or {}) do
		self.m_OtherPanelEntsData[k] = v
	end
...
function ENT:ShouldRenderClientEnts(transmit)
	return transmit and self:GetPos():DistToSqr(Trolleybus_System.EyePos()) < Trolleybus_System.GetPlayerSetting("TrolleybusDetailsDrawDistance") ^ 2 and not Trolleybus_System.RunEvent("Trolleybus_PreventRenderClientEnts", self)
end
...
function ENT:CreateClientEnts()
	...
	for k, v in pairs(self.m_ButtonsData) do
		...
		if v.model then button.MoveState = button.LastState and 1 or 0 end
	end
	...
	for i = 1, math.min(self:GetPassCount(), Trolleybus_System.GetPlayerSetting("TrolleybusMaxPassengers")) do
		...
		if pass then table.insert(self.Passengers, pass) end
	...
	if self.TrailerData then
		...
		if dt.creaksound then
			...
			hook.Add("Think", self.Joint, function(ent)
				if not ent.LastCurvature then return end
				local dt = FrameTime()
		...
		hook.Add("PreRender", self.Joint, function(ent)
			ent.ShouldDraw = IsValid(self) and IsValid(self:GetTrailer()) and not self:IsDormant() and not self:GetTrailer():IsDormant()
			ent:SetNoDraw(not ent.ShouldDraw)
		end)
		...
		self.Joint:AddCallback("BuildBonePositions", function(ent, count)
			if not ent.ShouldDraw then return end
...
function ENT:ClearClientEnts()
	...
	if not self.IsTrailer then SafeRemoveEntity(self.Steer) end
	if self.Doors then
	...
	if self.TrailerData then SafeRemoveEntity(self.Joint) end
		...
		if self.HasPoles and self.PolesData then
			...
			for i = 1, 2 do
				...
				if state == 0 then
					...
					if svcontact == contact and svwire == wire and svendpos == endpos then t.clcontactwiretime = RealTime() end
					if not contact or not t.clcontactwiretime or RealTime() - t.clcontactwiretime > 0.3 then
					...
					if data then
						...
						if statetime < 0.25 then
							...
							if object then wheelsnd = object:GetWheelSlidingSound(svwire) end
						end
					end
				else
					...
					if state == 3 then
						...
						if contact then
							...
							if wire then
								...
								if data then wheelang = data.Ang end
							end
						end
					end
				end
				...
				if TrolleyPoleDrawRotate and not TrolleyPoleDrawRotate:IsZero() then
				...
				if anim then
					...
					for k, v in pairs(anim) do
						...
						if istable(v) then type, mp, add = v[1], v[2] or mp, v[3] or add end
						if type == "Static" then
				...
				if TrolleyPoleWheelRotate and not wheelang and not TrolleyPoleWheelRotate:IsZero() then
				...
				if TrolleyPoleWheelOffset and not TrolleyPoleWheelOffset:IsZero() then wheelpos = wheelpos + wheellang:Forward() * TrolleyPoleWheelOffset.x - wheellang:Right() * TrolleyPoleWheelOffset.y + wheellang:Up() * TrolleyPoleWheelOffset.z end
...
function ENT:ControlLighting()
	if not self.IsTrailer then
		for k, v in ipairs(self.HeadLights) do
			if not v.ang then continue end
			if self.RenderClientEnts and self:GetHeadLights() > 0 and not Trolleybus_System.GetPlayerSetting("DisableHeadlights") and (not v.shouldbeactive or v.shouldbeactive(self)) then
				if not self.HeadLamps[k] then
		...
		if not dlight and self.RenderClientEnts and self:GetCabineLight() > 0 then
			local data = self.CabineLightData
			if not self.CabineLight then
	...
	if self.Buttons then
		for k, v in pairs(self.Buttons) do
			...
			if v.LastState ~= self:ButtonIsDown(k) then
				...
				if v.Ent then self:ProcessCfgSound(v.Ent, cfg.model.sounds, v.LastState) end
			end

			if cfg.think then cfg.think(self, v.LastState) end
			if cfg.model and (v.LastState and v.MoveState ~= 1 or not v.LastState and v.MoveState ~= 0) then
...
function ENT:GetDoorsSpeed(name)
	...
	if self.SystemsLoaded then
		for k, v in pairs(self.Systems) do
			if v.ModifyDoorsSpeed then mp = mp * (v:ModifyDoorsSpeed(name) or 1) end
		end
	end
	return mp
end
...
function ENT:ControlDoors()
	if self.Doors then
		...
		for k, v in pairs(self.Doors) do
			...
			v.State = state
			if v.State and v.MoveState ~= 1 or not v.State and v.MoveState ~= 0 then
			...
			if state and oldmove == 0 and opensoundstart then self:ProcessCfgSound(v, opensoundstart, state) end
			if not state and oldmove == 1 and closesoundstart then self:ProcessCfgSound(v, closesoundstart, state) end
			if movesound and (state and oldmove == 0 or not state and oldmove == 1) then
			...
			if state and oldmove ~= 1 and v.MoveState == 1 and opensoundend then self:ProcessCfgSound(v, opensoundend, state) end
			if not state and oldmove ~= 0 and v.MoveState == 0 and closesoundend then self:ProcessCfgSound(v, closesoundend, state) end
			if state and v.MoveState == 1 and oldmove ~= 1 or not state and v.MoveState == 0 and oldmove ~= 0 then
...
function ENT:ControlTurnSound()
	...
	if oldstate == nil then oldstate = self.TurnSignalSoundState end
	if not self.RenderClientEnts then return end
	local state = self.TurnSignalSoundState
	if oldstate ~= state then Trolleybus_System.PlayBassSoundSimple(self, state and self.TurnSignalSoundOn or self.TurnSignalSoundOff, 100, 1, nil, self.DriverSeatData.Pos) end
end
...
function ENT:ControlViewMove()
	...
	if Trolleybus_System.IsControlButtonDown("viewmove") and Trolleybus_System.CanPressButtons(self, ply) then
		...
		if ply:KeyDown(IN_JUMP) then dir:Add(ang:Up()) end
...
function ENT:ControlRollSound()
	if self.RenderClientEnts then
		...
		if not self.RollSoundPatch:IsPlaying() then
...
function ENT:RenderClientEntsChanged(state)
	if state then
		...
		if self.SpawnSettingsReceived then self:SetupSpawnSettingsOnClientEnts(self._SpawnSettings) end
		self:UpdateSkins()
	else
		self:ClearClientEnts()
	end
...
function ENT:SetupSpawnSettings()
	if self._SpawnSettings then
		for k, v in ipairs(self._SpawnSettings) do
			if self.SpawnSettings and self.SpawnSettings[k] and self.SpawnSettings[k].unload then self.SpawnSettings[k].unload(self, v) end
		end
	end
	...
	if self.SpawnSettings then
		for k, v in ipairs(self.SpawnSettings) do
			...
			if v.type == "ComboBox" and v.choices[v.default].value ~= nil then default = v.choices[v.default].value end
			local value = self:GetNWVar("SpawnSetting_" .. k, default)
			self._SpawnSettings[k] = value
			if v.alias then self._SpawnSettingsNames[v.alias] = k end
...
function ENT:OnSpawnSettingsReceived(settings)
	if not self.SystemsLoaded then self:SetupSystems() end
	if self.RenderClientEnts then self:ReloadClientEnts() end
end

function ENT:SetupSpawnSettingsOnClientEnts(settings)
	for k, v in ipairs(self.SpawnSettings) do
		if settings[k] == nil then continue end
		if v.setupclents then v.setupclents(self, settings[k]) end
	end
end
...
function ENT:UpdateClientEntsDistanceControl(clear)
	...
	if clear == nil then clear = not self.RenderClientEnts end
	local i = 1
	while true do
		...
		if clear or data.distance and eyepos:DistToSqr(ent and ent:GetPos() or self:LocalToWorld(data.pos)) > data.distance ^ 2 then
			if ent then
				...
				if data.otherent and IsValid(data.m_mirror) then data.m_mirror:Remove() end
			end
			...
			if not ent then
				...
				if data.otherent and self.MirrorsData then
					for k, v in pairs(self.MirrorsData) do
						if not v.otherent or v.otherent ~= data.otherent.index then continue end
						local mirror = Trolleybus_System.CreateMirror()
						mirror:SetWidth(v.width)
						mirror:SetHeight(v.height)
...
function ENT:CreateCustomClientEnt(index, model)
	if IsValid(self.CustomClientEnts[index]) then self.CustomClientEnts[index]:Remove() end
...
function ENT:Think()
	...
	if self.SpawnSettingsReceive ~= spawnsettingsreceive and (not self.IsTrailer or IsValid(self:GetTrolleybus())) then
		self.SpawnSettingsReceive = spawnsettingsreceive
		if self.IsTrailer then self.SpawnSettings = self:GetTrolleybus().SpawnSettings end
	...
	if self.RenderClientEnts and self.LastSkinsUpdate and CurTime() - self.LastSkinsUpdate > 0.25 then self:UpdateSkins(true) end
...
function ENT:OnRemove()
	...
	if oldrender ~= self.RenderClientEnts then self:RenderClientEntsChanged(self.RenderClientEnts) end
	if self.SystemsLoaded then
		for k, v in pairs(self.Systems) do
